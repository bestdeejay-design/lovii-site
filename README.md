...
```bash
python3 scripts/check-lovii-site.py --strict
```

Страж ловит три вещи, которые уже ломались на этом сайте:

1. **Сырые цвета** в `assets/site.css` — всё должно быть на токенах системы.
2. **Размеры иконок вне ступеней** 12/16/18/20/24/32.
3. **Устаревший снапшот ДС** — сверяет шапку `assets/lovii.css` с текущим каноном
   (проверка работает, если рядом с сайтом есть репозиторий `lovii-design`; иначе пропускается).

Полоса доверия в шапке главной собрана из чипов системы (24 px, пилюля, нейтральный тон) —
раньше там смешивались обычный текст и чипы.
